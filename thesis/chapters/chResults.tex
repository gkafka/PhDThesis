...
Nuisance parameters, including $\theta_{23}$ and the systematics, are handled by including and profiling them in equations \ref{eq:FCRank} and \ref{eq:FCConf} \cite{ref:FCNotes}.
\beq
R_{i} = P\left( x_i | x_e\left(\vec{\theta}, \vec{\lambda}'_{BF}\right) \right) / P\left( x_i | x_e\left(\vec{\theta}_{BF}, \vec{\lambda}_{BF}\right) \right)
\label{eq:FCRankLambda}
\eeq

\n In the numerator, $\vec{\lambda}'_{BF}$ is found by holding $\vec{\theta}$ constant and fitting the nuisance parameters to the possible observation, $x_i$. On the other hand, in the denominator both $\vec{\theta}$ and $\vec{\lambda}$ are fit to the possible observation, $x_i$, to find $\vec{\theta}$ and $\vec{\lambda}_{BF}$. In other words,
\beq
...
\n The significance, $c$, of a particular point $\vec{\theta}$ is calculated by summing the Poisson probabilities of observing $x_i$ events given the fixed $\vec{\theta}$ and nuisance parameters best fit to the data.
\beq
...
\beq
\vec{\lambda}_{BF(Data)} = \vec{\lambda} | \chi^2\left(x_{Data}, \vec{\theta}, \vec{\lambda}\right) = \chi^2_{Min}\left(x_{Data}, \vec{\theta}\right),
\label{eq:FCLambdaData}
\eeq

\n Note than in equation \ref{eq:FCConfLambda}, both $\vec{\theta}$ and $\vec{\lambda}_{BF(Data)}$ are held constant. This must be the case so that summing over all $i$ would result in $c = 1$. Conversely, the nuisance parameters are fit to the possible observation, $x_i$, when calculating the ranks, $R_i$, as shown in equation \ref{eq:FCLambdaNum}.
